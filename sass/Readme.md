...
folder are mapped to the application's root namespace, 'MassEvents'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "MassEvents"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'MassEvents' in the same way
as `"MassEvents/sass/src"`.
