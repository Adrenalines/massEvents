Ext.define("MassEvents.view.trees.mainTreesPanController", {
      ...
      "button[name=searchHelpButton]": {
        afterrender: function(element) {
          Ext.create("Ext.tip.ToolTip", {
            ...
            html:
              "Горячие клавиши:<br />Enter - осуществить поиск<br />Esc - очистить поле ввода.",
            autoHide: true,
            closable: true,
            draggable: true
          });
  ...
  onSearch: function(searchBut, search, techIndex) {
    ...
    if (!tabPanel) {
      ...
      tree = Ext.ComponentQuery.query(
        "treepanel[name=" + tabPanel.getActiveTab().name + "]"
      )[0];
    }
    ...
    if (textSearch.validate()) {
      ...
      if (
        tree.getItemId() == "checkSrvList" ||
        tree.getItemId() == "checkSrvListLoad"
      ) {
        ...
        Ext.Array.each(storeRange, function(item1, i) {
          ...
          Ext.Array.each(item1.childNodes, function(item2, j) {
            if (
              item2.data.text.toLowerCase().includes(checkText.toLowerCase())
            ) {
              data[k] = {
                obj: ""
              };
        ...
        if (data.length == 0) {
          ...
          if (data.length === 1) {
            searchCombo.fireEvent("select", searchCombo, [
              searchCombo.getStore().getAt(0)
            ]);
          } else {
            searchCombo
              .show()
          ...
          success: function(response, me) {
            ...
            if (data.failure) {
              ...
              if (data.msg === "") {
                ...
                Ext.Msg.alert(
                  "Сервер",
                  "Проблемы с сервером, повторите попытку позже"
                );
              }
            } else {
              textSearch.setLoading(false);
  ...
  getSearchObj: function(combo, records) {
    ...
    if (tree.getItemId() == "btsCellTreeLoad") {
    ...
    } else if (tree.getItemId() == "plTreeLoad") {
      let searchNode;
      if (typeof records[0] != "undefined") {
        textSearchNodeObj = records[0].data;
      } else {
      ...
      Ext.Ajax.request({
        ...
        success: function(response) {
          ...
          if (resp.failure != true) {
            // не передаём but, т.к. это поиск
            self.extractPl(resp[0].obj, textSearchNodeObj.obj);
          }
        }
      });
  ...
  getObjects: function(but, selReg, treeName) {
    ...
    Ext.Array.each(selReg, function(item, i) {
      if (treeName == "slaTreeLoad") {
        objArr.push(
          selReg[i].data.name.substring(
            selReg[i].data.name.indexOf(":") + 1,
            selReg[i].data.name.length
          )
        );
      } else if (treeName == "geoTreeLoad" || treeName == "plTreeLoad") {
  ...
  // формируем URL для ajax-запроса на поиск bts и сот внутри объекта
  getUrlString: function(treeName) {
    ...
    } else if (treeName == "plTreeLoad") {
      callString = "plBtsCell";
    }
  ...
  getAjaxBtsCell: function(
    ...
    let self = this;
    if (treeName == "geoTreeLoad" || treeName == "plTreeLoad") {
      ...
      success: function(response, opts) {
        ...
        if (resp.failure != true) {
          ...
          if (i < objArr.length - 1) {
            ...
            if (
              btsArr.length + cellArr.length > 5000 ||
  ...
  loadBtsCell: function(but, treeName, btsArr, parentBtsArr, cellArr) {
    ...
    Ext.Array.each(btsArr, function(item, i) {
      if (treeName == "slaTreeLoad") {
        ...
        treeName == "geoTreeLoad" ||
        treeName == "regionTreeLoad" ||
        treeName == "plTreeLoad"
      ) {
        // вызываем метод извлечения BTS
        self.extractBtsCell(
  ...
  extractBtsCell: function(
    ...
    let self = this,
      ...
      tree = Ext.ComponentQuery.query("treepanel[name=btsCellTreeLoad]")[0],
      node = tree.getRootNode(),
      cellIsPressed = Ext.ComponentQuery.query(
    ...
    if (searchNode != null) {
      ...
      if (
        nodeToAppend.findChild("checked", true, true) == null &&
        ((treeName == "slaTreeLoad" && btsOrCellForSla == "bts") ||
          ((treeName == "geoTreeLoad" ||
            treeName == "regionTreeLoad" ||
            treeName == "plTreeLoad") &&
        ...
        (treeName == "slaTreeLoad" && btsOrCellForSla == "cell") ||
        ((treeName == "geoTreeLoad" ||
          treeName == "regionTreeLoad" ||
          treeName == "plTreeLoad") &&
          cellIsPressed == true)
          ...
          success: function(response) {
            ...
            if (treeName == "slaTreeLoad" && btsOrCellForSla == "cell") {
              ...
              if (findedNode != null) {
                ...
                if (
                  findedNode.data.name == arr[arr.length - 1] ||
                  findedNode.data.name == arr
                ) {
      ...
      if (
        ((treeName == "slaTreeLoad" && btsOrCellForSla == "bts") ||
          ((treeName == "geoTreeLoad" ||
            ...
            treeName == "plTreeLoad") &&
            cellIsPressed == false)) &&
        (nodeToAppend.data.name == arr[arr.length - 1] ||
  ...
  extractPl: function(searchObj, appendObj) {
    let searchNode,
      textSearchNode = {},
      tree = Ext.ComponentQuery.query("treepanel[name=plTreeLoad]")[0],
      node = tree.getRootNode();
    searchNode = node.findChild("text", searchObj, true);
    if (searchNode != null) {
      if (searchNode.data.expanded == false) {
  ...
  backIfError: function(but) {
    Ext.Msg.alert(
      "Полученное количество BTS / CELL слишком велико. ",
